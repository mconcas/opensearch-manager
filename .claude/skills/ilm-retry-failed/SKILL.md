---
name: ilm-retry-failed
description: Retry failed OpenSearch ISM steps. Trigger when `ilm-diagnose` shows indices in step_status=failed, or the user says "ISM step failed" / "policy step keeps failing" / "stuck in retry". Calls `_plugins/_ism/retry/<idx>` to push the index out of failed state.
---
...
After `ilm-diagnose` reports indices with `step_status: failed` AND `action.failed: true` (the latter is the precondition for the retry endpoint to do anything).
...
- `step_status: condition_not_met` is NOT a failure — it's a polling step. The retry endpoint won't help. Fix is at the policy level (e.g. add `min_index_age` to rollover) or via `ilm-upgrade-policy --state` to bump to a different state.
...
If the table is empty, there's nothing for this skill to do — re-route to `ilm-diagnose`.

### 2. Read the underlying error

The `Errors:` section gives the actual cause. Common ones:
- "is the write index for data stream … and cannot be deleted" → resolve the data-stream block first via `ilm-rollover-write-index`, then retry.
...
For a single index:
```bash
starsearch-cli ilm retry '<index-name>'
```

For a pattern (comma-separated or wildcard accepted by ISM):
```bash
starsearch-cli ilm retry '<pattern>'
```

To retry from a specific state in the policy (rare — usually you want the natural state):
```bash
starsearch-cli ilm retry '<pattern>' --state <state>
```

### 4. Verify

```bash
starsearch-cli ilm status '<index>'
```
...
- `_plugins/_ism/retry` does NOT pick up a queued `change_policy`. If you want to both reset the state AND apply a new policy version, use `starsearch-cli ilm change-policy <idx> <policy> --state <state>` instead.

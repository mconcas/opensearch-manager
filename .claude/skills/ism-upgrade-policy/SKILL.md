---
name: ism-upgrade-policy
description: Re-enrol OpenSearch indices on the latest version of an ISM policy. Trigger when `ism-diagnose` (or `osm ism policy version`) shows indices "drifted" (policy_seq_no older than current) or "not enrolled" (policy_id set but no managed_index doc), and the user wants the updated policy to actually take effect. Mutating — requires user confirmation.
---
...
After `ism-diagnose` reports:
- Drifted indices (policy_seq_no behind current).
- Not-enrolled indices (policy_id setting present, no managed_index doc).

Both are addressed by `osm ism change-policy`, which transparently falls back to `_plugins/_ism/add` when ISM reports an index as "not being managed".
...
```bash
osm ism change-policy '<pattern>' <policy-name>
```
...
```bash
osm ism change-policy '<index>' <policy-name> --state <state>
```

Use this sparingly — it bypasses ISM's natural state machine. Common case: an index in `delete/failed` whose underlying block has been cleared (e.g. after `ism-rollover-write-index`) but ISM has stopped auto-retrying.

### 4. Be aware: convergence is NOT immediate

The change is **queued**, not applied. Apparent `policy_seq_no` (per `ism policy version`) updates after ISM next executes a step for that index. The per-index schedule interval is baked in at managed-index creation — typically 60 min — and is **not** affected by changing `plugins.index_state_management.job_interval` on the cluster.
...
- For indices with `policy_id: null` and no `ism_template` match, change_policy / add will succeed but you may want to also fix the underlying template (see `ism-template-coverage` for the template-side fix).
